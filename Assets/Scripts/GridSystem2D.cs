...
public class GridSystem2D<T> 
{
    ...
    void DrawDebugLines() {
        ...
        for(int x = 0; x < width; x++) {
            for(int y = 0; y < height; y++) {
                ...
                Debug.DrawLine(GetWorldPosition(x, y), GetWorldPosition(x + 1, y), Color.blue, duration);
            }
        }

        Debug.DrawLine(GetWorldPosition(0, height), GetWorldPosition(width, height), Color.green, duration);
        Debug.DrawLine(GetWorldPosition(width, 0), GetWorldPosition(width, height), Color.red, duration);
    }
    ...
    public class VerticalConverter : CoordinateConverter {
        ...
        public override Vector3 GridToWorldCenter(int x, int y, float cellSize, Vector3 origin) {
            return new Vector3(x * cellSize + cellSize * 0.5f, y * cellSize + 0.5f, 0) + origin;
        }
    ...
    /// <summary>
    /// A coordinate converter for horizontal grids, where the grid lies on the X-Y plane.
    /// </summary>
    ...
    public class HorizontalConverter : CoordinateConverter {
        ...
        public override Vector3 GridToWorldCenter(int x, int y, float cellSize, Vector3 origin) {
            return new Vector3(x * cellSize + cellSize * 0.5f, 0, y * cellSize + 0.5f) + origin;
        }
